        ...
        <button className="btn btn-ghost btn-circle" onClick={()=>document.getElementById('my_modal_1').showModal()}>
          ...
          <dialog id="my_modal_1" class="modal">
            <div class="modal-box">
                ...
                <input type="text" className="w-full" placeholder="Search" value={search} onChange={(e) => setSearch(e.target.value)}/>
                <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 16 16"
                fill="currentColor"
                ...
                <div className="flex flex-col gap-4 items-start w-full text-left">
                    <button className="btn btn-ghost w-full" onClick={() => handleClick('articles')}>Articles: {search}</button>
                    <button className="btn btn-ghost w-full" onClick={() => handleClick('users')} >Username: {search}</button>
