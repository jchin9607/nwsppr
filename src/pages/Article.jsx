  ...
  useEffect(() => {
    if (!cachedData) {
      ...
      if (cachedData.date instanceof Timestamp) {
        setDate(
          cachedData.date.toDate().toLocaleDateString("en-us", {
            year: "numeric",
            month: "long",
            day: "numeric",
            hour: "numeric",
    ...
    <div className="w-full flex justify-center mb-[20vh] mt-[10vh] sm:mt-0">
      ...
      <div className="flex flex-col w-[80vw] items-center justify-start min-h-screen pt-[5%]">
        <div className="prose prose-sm sm:prose lg:prose-lg w-full">
          ...
          <div className="text-lg mt-[30px] md:text-xl">
        ...
        <div className="prose prose-sm sm:prose lg:prose-lg w-full mb-[20px]">
          {cachedData.tags &&
            cachedData.tags.map((tag) => (
              <span className="badge badge-accent mr-2">#{tag}</span>
            ))}
        </div>
        ...
        <div
          className="prose prose-sm sm:prose lg:prose-lg  break-words w-full"
          dangerouslySetInnerHTML={{ __html: cachedData.content }}
        ></div>
      </div>
    </div>
  );
