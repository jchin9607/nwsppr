...
const Home = ({ userId }) => {
  ...
  const [articles, setArticles] = useState(
    JSON.parse(localStorage.getItem("articlesList")) || null
  );
  ...
  function handleSetFilterFollowing() {
    setFilterFollowing(data.data().following);
  }
        ...
        <div className="flex-col gap-[10px] sticky top-[60px] hidden h-[50%] md:flex md:w-[25%] lg:flex lg:w-1/6  ">
          ...
          <div>
            ...
            <span
              ...
              onClick={() => handleSetFilterFollowing()}
            >
              Following
            </span>
            ...
            {filterList.map((filter) => (
              <span
                className={`badge badge-accent mr-2 cursor-pointer ${
                  currentFilter === filter && "text-white"
                }`}
                key={filter}
              >
        ...
        <div className="w-full lg:w-7/12">
          ...
          <div className="dropdown dropdown-hover">
            <div tabIndex={0} role="button" className="btn m-1">
              Sort By: {sort ? "Trending <14" : "Newest"}
            </div>
            <ul
              tabIndex={0}
              className="dropdown-content menu bg-base-100 rounded-box z-[1] w-52 p-2 shadow"
            >
              <li onClick={() => handleSort(true)}>
                <a>Trending</a>
              </li>
              <li onClick={() => handleSort(false)}>
                <a>Newest</a>
              </li>
            </ul>
          </div>
          ...
          {!loading &&
            ...
            articles.docs
              ...
              .filter(
                (doc) =>
                  filterFollowing?.includes(doc.author) ||
                  filterFollowing === null
              )
